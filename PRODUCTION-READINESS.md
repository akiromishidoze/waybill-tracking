...
44. ~~**Terraform remote state** — `infrastructure/terraform/` configs store state locally. No S3/GCS backend.~~ ✅ Done — Enabled partial `backend "s3" {}` configuration in `infrastructure/terraform/aws/main.tf` and `backend "gcs" {}` in `infrastructure/terraform/gcp/main.tf`. Updated `backend.tfbackend.example` files to be copy-paste ready templates. Added bootstrap modules (`backend/aws-bootstrap` and `backend/gcs-bootstrap`) for creating the state storage buckets, and added `infrastructure/terraform/README.md` with setup and migration instructions. Added `backend.tfbackend` and `terraform.tfstate.*.backup` to `.gitignore`.
